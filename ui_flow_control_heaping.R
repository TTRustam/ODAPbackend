# NOTE!!!!!!! I have updated this flowcontrol
# now since we treat modules as standalone versions,
# I have prepared some new check heaping examples
# we now have an updated general heaping function 
# it now can take rates, proportions and counts
# the variable name that we want to check is assigned with y argument
...
data_in     <- data_in[data_in$.id != 2, ]

# # standard checks run before doing the thing.
initial_data_checks <- check_upper(data = data_in, module = "generic")
initial_data_checks

# the outputs are the same as before, only function arguments have changed
# Note the first one operates with counts
check_heaping_general(data = data_in, y = "Deaths", units = "count")
# this one uses the nMx - rate
# NOTE: The treshholds for rates and proportions are not yet determined
# so migt returnNA in level and color. this will be fixed later
# have user specify which column is to be checked for heaping.
# i.e. just one column at a time; the same heaping measures could
# be used for any count data. Also make them specify what units are used
